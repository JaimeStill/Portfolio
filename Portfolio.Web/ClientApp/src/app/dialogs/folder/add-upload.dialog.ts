...
export class AddUploadDialog implements OnInit {
  ...
  ngOnInit() {
    this.upload.getExcludedUploads(this.data.userId, this.data.name);
  }
  ...
  addNewUploads = async () => {
    this.uploading = true;
    const res = await this.upload.uploadFiles(this.formData, this.data.userId);
  ...
  addFolderUploads = async (u: Upload[]) => {
    this.uploading = true;
    const uploads = u.map(x => Object.assign({ } as FolderUpload, {
